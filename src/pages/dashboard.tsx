...
import { useRoqPlatformUserProfiles, useProjectCount, useProjectTaskCount, useKanbanBoardCount } from 'lib/roq';

import { KanbanBoardListPage as ShowCaseList } from 'pages/kanban-boards';
...
function DashboardPage() {
  ...
  const projectTaskStat = useProjectTaskCount();

  const kanbanBoardStat = useKanbanBoardCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Project Tasks',
      result: projectTaskStat?.data,
      isLoading: projectTaskStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Kanban Boards',
      result: kanbanBoardStat?.data,
      isLoading: kanbanBoardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Kanban Boards
                </Text>
                <NextLink href={`kanban-boards`} passHref legacyBehavior>
                  <Button
